Attribute VB_Name = "modArielBrowseFolder"
'----------------------------------------------------------------------------------
...
Private Function CheckSpecialFolder(FolderId As SpecialFolders) As String
'------------------------------------------------------------------------------
'Check the location of a special folder through API, given its CSID
'Return its path if it exists, or "" if not
'Note: THis function is not used in this implementation and is only provided
'      for reference purposes.
'------------------------------------------------------------------------------
Dim sPath As String
Dim lRet As Long

'Fill our string buffer to receive the folder path
sPath = String(MAX_PATH, 0)
'Get the path
'SHGetFolderPath Lib "shfolder" Alias "SHGetFolderPathA" _
...
'lRet = SHGetFolderPath(0, FolderId Or CSIDL_FLAG_CREATE, 0, SHGFP_TYPE_CURRENT, sPath)
Select Case lRet
Case C_OK
  'We retrieved the folder successfully
  'All C strings are null terminated, so strip trailing nulls
  sPath = Left(sPath, InStr(1, sPath, Chr(0)) - 1)
  CheckSpecialFolder = sPath
Case C_FALSE
  'The CSIDL in FolderId is valid, but the folder does not exist.
